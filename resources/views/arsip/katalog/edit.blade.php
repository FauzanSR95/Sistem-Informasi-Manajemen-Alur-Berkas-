<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-md sm:rounded-xl border border-gray-200">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('arsip.katalog.update', $katalog) }}" class="mt-6 space-y-6">
                        @csrf
                        @method('PATCH')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-label for="nomor_hm" value="Nomor HM (Hak Milik)" />
                                <x-text-input id="nomor_hm" name="nomor_hm" type="text" class="mt-1 block w-full {{ $errors->has('nomor_hm') ? 'border-red-500' : '' }}" :value="old('nomor_hm', $katalog->nomor_hm)" required />
                            ...
                            <div>
                                ...
                                <select name="jenis_sertifikat" id="jenis_sertifikat" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">
                                    ...
                                    {{-- Opsi digabung, dan akan otomatis terpilih jika data lama adalah SU, BT, atau SU - BT --}}
                                    <option value="SU - BT" @selected(in_array(old('jenis_sertifikat', $katalog->jenis_sertifikat), ['SU - BT', 'SU', 'BT']))>
                                        SU - BT
                                    </option>
                                    
                                    <option value="HT" @selected(old('jenis_sertifikat', $katalog->jenis_sertifikat) == 'HT')>HT</option>
                                    <option value="HGB" @selected(old('jenis_sertifikat', $katalog->jenis_sertifikat) == 'HGB')>HGB</option>
                                    <option value="HGU" @selected(old('jenis_sertifikat', $katalog->jenis_sertifikat) == 'HGU')>HGU</option>
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="lokasi_rak" name="lokasi_rak" type="number" min="1" class="mt-1 block w-full" :value="old('lokasi_rak', $katalog->lokasi_rak)" required />
                            </div>
                            <div>
                                <x-input-label for="lokasi_baris" value="Lokasi Baris" />
                                <x-text-input id="lokasi_baris" name="lokasi_baris" type="number" min="1" class="mt-1 block w-full" :value="old('lokasi_baris', $katalog->lokasi_baris)" required />
                            </div>
                        </div>
    ...
    <script>
        ...
        // Ambil data lama dari objek $katalog
        const oldKecamatan = "{{ old('kecamatan', $katalog->kecamatan) }}";
        const oldDesa = "{{ old('desa', $katalog->desa) }}";
        ...
        function populateDesa(selectedKecamatan, preselectedDesa = null) {
            ...
            if (selectedKecamatan && dataWilayah[selectedKecamatan]) {
                ...
                listDesa.sort().forEach(function(desa) {
                    ...
                    option.value = desa;
                    option.text = desa;
                    if (desa === preselectedDesa) option.selected = true; 
        ...
        // Isi dropdown Kecamatan saat halaman dimuat
        ...
        // Jika ada data lama (sedang edit), langsung render desa-nya
        if (oldKecamatan) {
