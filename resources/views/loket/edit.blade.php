<x-app-layout> 
    <x-slot name="header"> 
        <h2 class="font-semibold text-xl text-gray-800 leading-tight"> 
            {{ __('Edit & Revisi Pengajuan Berkas #'.$pengajuan->id) }} 
        </h2> 
    </x-slot> 
            ...
            <div class="bg-white overflow-hidden shadow-md sm:rounded-xl border border-gray-200"> 
                <div class="p-6 text-gray-900"> 
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-bold text-lg">Formulir Revisi Berkas</h3> 
                        <a href="{{ route('loket.alur') }}" class="text-sm text-gray-600 hover:underline">&larr; Kembali ke Alur</a>
                    </div>
                    ...
                    {{-- Form mengarah ke update dengan method PUT --}} 
                    <form id="pengajuan-form" method="POST" action="{{ route('loket.update', $pengajuan) }}" class="mt-6 space-y-6" novalidate> 
                        @csrf 
                        @method('PUT')
 
                        <div> 
                            <x-input-label for="nama_pemohon" :value="__('Nama Pemohon')" /> 
                            <x-text-input id="nama_pemohon" name="nama_pemohon" type="text" class="mt-1 block w-full" value="{{ old('nama_pemohon', $pengajuan->nama_pemohon) }}" autofocus /> 
                            <p id="nama_pemohon_error" class="text-sm text-red-600 mt-2"></p> 
                        </div> 
 
                        <div> 
                            <x-input-label for="nomor_hak" :value="__('Nomor Hak')" /> 
                            <x-text-input id="nomor_hak" name="nomor_hak" type="number" class="mt-1 block w-full" value="{{ old('nomor_hak', $pengajuan->nomor_hak) }}" /> 
                        ...
                        <div> 
                            ...
                            <textarea id="catatan_loket" name="catatan_loket" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full">{{ old('catatan_loket', $pengajuan->catatan_loket) }}</textarea> 
                        </div> 
 
                        <div class="flex items-center gap-4"> 
                            <x-primary-button>{{ __('Simpan Perubahan & Kirim Ulang') }}</x-primary-button> 
                        </div> 
                    </form> 
                </div> 
            </div> 
    ...
    <script> 
        ...
        // Ambil data lama dari database untuk auto-select
        const oldKecamatan = "{{ old('kecamatan', $pengajuan->kecamatan) }}";
        const oldDesa = "{{ old('desa', $pengajuan->desa) }}";

        // Fungsi untuk mengisi desa berdasarkan kecamatan
        function populateDesa(selectedKecamatan, preselectedDesa = null) {
            ...
            if (selectedKecamatan && dataWilayah[selectedKecamatan]) {
                ...
                listDesa.sort().forEach(function(desa) {
                    ...
                    option.value = desa;
                    option.text = desa;
                    if (desa === preselectedDesa) option.selected = true; // Auto select desa lama
        ...
        for (let kec in dataWilayah) {
            ...
            option.value = kec;
            option.text = kec;
            if (kec === oldKecamatan) option.selected = true; // Auto select kecamatan lama
            kecamatanSelect.appendChild(option);
        }

        // Jika halaman pertama dimuat dan ada data lama, langsung isi desa-nya
        if (oldKecamatan) {
            populateDesa(oldKecamatan, oldDesa);
        }

        // Event listener saat user mengganti kecamatan
